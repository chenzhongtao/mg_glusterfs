...
//·ÖÅärpcsvc_notify_wrapper_tÄÚ´æºÍ³õÊ¼»¯
...
rpcsvc_can_outstanding_req_be_ignored (rpcsvc_request_t *req)
{
         ...
         * right now, we should unwind the fop saying â€œrequest registered, will
...
rpcsvc_transport_create (rpcsvc_t *svc, dict_t *options, char *name)
{
        ...
        //×°ÔØ´«Êä²ãµÄ¿â²¢³õÊ¼»¯
        //¶¯Ì¬ÔØÈëÏàÓ¦ÀàÐÍµÄ RPC ¿â²¢µ÷ÓÃ¿âµÄ init ³õÊ¼»¯
        ...
        //½¨Á¢´«Êä²ãÁ´½Ó¼àÌý£¬Ö´ÐÐ×°ÔØ¿âºóµÄlistenº¯Êý
        //µ÷ÓÃ sokcet ¿âµÄ listen ½¨Á¢¼àÌý¶Ë¿Ú £¬ ²¢µ÷ÓÃ
        //ctx->event_pool µÄ event_register_epoll º¯Êý½« sokcet ¾ä±úÀû
        //ÓÃ epoll_ctl ¼àÌý
        ...
        //×¢²áÍ¨Öªº¯Êý,×¢²á trans µÄ notify ´¦Àíº¯Êý
...
rpcsvc_create_listener (rpcsvc_t *svc, dict_t *options, char *name)
{
        ...
        //¸ù¾Ý´«ÊäÃû³Æ´´½¨´«ÊäÃèÊö¶ÔÏó
        //´´½¨Ò»¸ö rpc_transport_t ½á¹¹Ìå£¬¸Ã½á¹¹Ìå¶¯Ì¬ÔØÈë soket
        //¿âÒÔ¼°Æäº¯ÊýÖ¸Õë£¬´´½¨Ò»¸ö socket
        ...
        //Îª¼àÌý¶ÔÏó·ÖÅä×ÊÔ´
        //´´½¨ listener£¬²¢½«¸Ã rpc server ºÍ trans ¸³Öµ¸ø listener¡£
        //½«¸Ã listener Ìí¼Óµ½ prc server µÄ svc->listeners Á´±íÉÏ
        ...
        ret = 0;
out:
        //Èç¹û·ÖÅä¼àÌý×ÊÔ´Ê§°Ü²¢ÇÒ´«ÊäÁ´½ÓÒÑ½¨Á¢³É¹¦
        if (!listener && trans) {
                //¶Ï¿ª´«ÊäÁ´½Ó
                rpc_transport_disconnect (trans);
        }
...
rpcsvc_register_notify (rpcsvc_t *svc, rpcsvc_notify_t notify, void *mydata)
{
        ...
        //ÎªÍ¨Öªº¯ÊýµÄ°ü×°¶ÔÏó·ÖÅä×ÊÔ´
        wrapper = rpcsvc_notify_wrapper_alloc ();
        if (!wrapper) {
                goto out;
        }
        svc->mydata   = mydata;  /* ÉèÖÃÊôÓÚÄÄÒ»¸öxlator*/
        //Í¨Öªº¯ÊýµÄ°ü×°¶ÔÏóÒ²±£´æÊôÓÚÄÄÒ»¸öxlator
        wrapper->data = mydata; 
        //±£´æÍ¨ÖªµÄ»Øµ÷º¯Êý
        wrapper->notify = notify; 

        //svcÊÇÈ«¾Ö¶ÔÏó£¬·ÃÎÊÐèÒª¼ÏËø
        pthread_mutex_lock (&svc->rpclock);
        {
                //Ìí¼Óµ½svcµÄÍ¨Öªº¯ÊýÁÐ±í
...
rpcsvc_init (xlator_t *xl, glusterfs_ctx_t *ctx, dict_t *options,
             uint32_t poolcount)
{
        rpcsvc_t          *svc              = NULL;//ËùÓÐrpc·þÎñµÄÈ«¾Ö×´Ì¬ÃèÊö¶ÔÏó
        int                ret              = -1;

        if ((!xl) || (!ctx) || (!options))
                return NULL;

        svc = GF_CALLOC (1, sizeof (*svc), gf_common_mt_rpcsvc_t);//·ÖÅäÄÚ´æ×ÊÔ´
        if (!svc)
                return NULL;

        pthread_mutex_init (&svc->rpclock, NULL);//³õÊ¼»¯Ëø
        INIT_LIST_HEAD (&svc->authschemes);//³õÊ¼»¯È¨ÏÞÄ£Ê½Á´±í
        INIT_LIST_HEAD (&svc->notify);//³õÊ¼»¯Í¨Öª»Øµ÷º¯ÊýÁ´±í
        INIT_LIST_HEAD (&svc->listeners);//³õÊ¼»¯¼àÌýÁ´±í
        INIT_LIST_HEAD (&svc->programs);//³õÊ¼»¯ËùÓÐ³ÌÐòÁ´±í

        ret = rpcsvc_init_options (svc, options);//³õÊ¼»¯rpc·þÎñµÄ¿ÉÑ¡ÏîÐÅÏ¢
        if (ret == -1) {
        ...
        ret = -1;
        svc->options = options; //¿ÉÑ¡Ïî±£´æ
        svc->ctx = ctx; //ËùÊôctx
        svc->mydata = xl;
        gf_log (GF_RPCSVC, GF_LOG_DEBUG, "RPC service inited.");

        //ÈÃÃèÊö³ÌÐòµÄ¶ÔÏó±£´æÑ¡ÏîÐÅÏ¢
        gluster_dump_prog.options = options; 

        //×¢²árpc·þÎñ,Ìí¼Ógluster_dump_prog µ½ svc->programs Á´±íÉÏ
...
free_svc:
        ...
        return svc;//·µ»Ø³õÊ¼»¯ºóµÄËùÓÐrpc·þÎñµÄÈ«¾ÖÃèÊö¶ÔÏó
}
