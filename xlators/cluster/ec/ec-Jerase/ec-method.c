...
size_t ec_method_encode(uint32_t k, uint32_t m, int32_t *matrix, uint32_t idx,
                        uint8_t * in, uint8_t * out, size_t size)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        {   
            for (j = 0; j < k; j++)
            {
                data_ptr[j] = in + i*EC_METHOD_CHUNK_SIZE*k + j*EC_METHOD_CHUNK_SIZE;
            }
            
            jerasure_matrix_dotprod_one(k, EC_METHOD_WORD_SIZE, matrix+((idx-k)*k),
                          (char **)data_ptr, (char *)out, EC_METHOD_CHUNK_SIZE);
        }
