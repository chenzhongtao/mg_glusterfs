                        ...
                        uint8_t * in, struct iobuf ** code_bufs, size_t size)
{
    ...
    {  
        for (j = 0; j < k; j++)
        {
            ...
            memcpy(code_bufs[j]->ptr+i*EC_METHOD_CHUNK_SIZE,  ptr_copy[j], EC_METHOD_CHUNK_SIZE);
        }
  
        for (j = 0; j < m; j++) 
            ptr_copy[j+k] = code_bufs[k+j]->ptr + i*EC_METHOD_CHUNK_SIZE;

        for (tdone = 0; tdone < EC_METHOD_CHUNK_SIZE; tdone += w_package_size) 
        {
            jerasure_do_scheduled_operations((char **)ptr_copy, smart, EC_METHOD_PACKAGE_SIZE);
            for (j = 0; j < k+m; j++) 
