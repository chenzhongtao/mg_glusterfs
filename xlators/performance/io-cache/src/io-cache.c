...
//ÅÐ¶ÏÊÇ·ñÓÐ»º´æ£¬²¢ÇÒÊÇ²»ÊÇÒªÐÞ¸ÄatimeºÍmtime£¬ÊÇ¾ÍÒªÏÈÊÍ·Å»º´æ
int32_t
                        ...
                        dict_t *xdata)
{
        ...
        //»º´æÒÑÊ§Ð§£¬mtime¸Ä±äÁË£¬±ØÐëÇå¿Õ»º´æ
        if ((op_ret == -1) ||
            ((op_ret >= 0) && !ioc_cache_still_valid(ioc_inode, stbuf))) {
                 ...
                 * the pages which have waiting frames by ioc_inode_wakeup()
                 Ö»ÓÐÃ»ÓÐ±»frameµÈ´ýµÄpage²Å»á±»__ioc_inode_flushº¯ÊýÊÍ·Å¡£ÕâÐ©Ò³
                 ½«ÔÚioc_inode_wakeupº¯ÊýÖÐ·¢ÉúÒ³´íÎó¡£
                 
                 */
                ioc_inode_lock (ioc_inode);
                {
                        //Çå¿ÕinodeµÄËùÓÐ»º´æ(ÏÂ¸ö¶Á»á´¥·¢Ò³´íÎó???)
                        // ´¥·¢´Ë²Ù×÷µÄpage²»»á±»ÊÍ·Å£¬ÒòÎª»¹ÓÐpage->waitq
                        destroy_size = __ioc_inode_flush (ioc_inode);
                        if (op_ret >= 0) {
                                ioc_inode->cache.mtime = stbuf->ia_mtime;
                ...
                ioc_inode_unlock (ioc_inode);
                // ÕâÀïÉèÖÃÎªNULL£¬ioc_inode_wakeupº¯ÊýÖÐ´¥·¢Ò³´íÎó
                local_stbuf = NULL;
        }
        ...
        //Ö»ÓÐº¯Êý·µ»ØÎª0£¬ÇÒ»º´æ»¹ÓÐÐ§£¬local_stbuf²»»á±»ÉèÖÃÎªNULL
        ...
        /* any page-fault initiated ·¢Æð by ioc_inode_wakeup() will have its own
...
ioc_wait_on_inode (ioc_inode_t *ioc_inode, ioc_page_t *page)
{
        ...
        // ÅÐ¶ÏpageÊÇ·ñÒÑÔÚinodeµÈ´ý¶ÓÁÐÖÐ
        while (trav) {
                if (trav->data == page) {
                        page_found = 1;
                        break;
...
//»ñÈ¡ÓÅÏÈ¼¶£¬Èç¹ûÃ»ÓÐ¶¨ÒåÓÅÏÈ¼¶Ä£Ê½£¬Ä¬ÈÏ¶¼ÊÇ1
uint32_t
ioc_get_priority (ioc_table_t *table, const char *path)
{
        uint32_t             priority = 1;
 ...
 */
 /*
ÒÔÏÂÈýÖÖÇé¿öµÄÎÄ¼þµÄ²»»º´æ £º
1.ÎÄ¼þ´óÐ¡²»·ûºÏ¹æ¶¨´óÐ¡£¬Ä¬ÈÏÊÇËùÓÐÎÄ¼þ¶¼Ö§³Ö£»
2.ÎÄ¼þ´ò¿ªÊ±£¬ÊÇÖ±Ð´£¬Ìø¹ý»º´æ£¬O_DIRECT;
3.ÎÄ¼þµÄÓÅÏÈ¼¶Îª0£¬Ä¬ÈÏÊÇ1£»
*/
int32_t
              ...
              int32_t op_errno, fd_t *fd, dict_t *xdata)
{
        ...
        if (op_ret != -1) {
                ...
                ioc_table_lock (ioc_inode->table);
                {
                        // É¾³ýºóÌí¼Óµ½Î²²¿£¬¸ù¾ÝlruËã·¨£¬¸ÕÊ¹ÓÃµ½µÄÒªÒÆ¶¯µ½Î²²¿
                        list_move_tail (&ioc_inode->inode_lru,
                                        &table->inode_lru[ioc_inode->weight]);
                }
                ioc_table_unlock (ioc_inode->table);
                ...
                struct iatt *postparent, dict_t *xdata)
{
        ...
        if (op_ret != -1) {
                ...
                // ´´½¨Ò»¸öÐÂµÄioc_inode£¬²¢°ÑËü¼ÓÈëµ½tableÖÐ
 ...
 *
 */
 // °Ñloc,inode,flag ±£´æÆðÀ´£¬»Øµ÷º¯Êý»áÓÃµ½
int32_t
                       ...
                       off_t offset, size_t size)
{
        ...
        local->wait_count++; // ioc_frame_return ÓëÕâ¸öÓÐ¹Ø£¬µÈ´ýËùÓÐµÄpage wait·µ»Ø
         ...
         * 2. In-transit - page fault has been generated on this page, we need
         ÔÚÍ¾ÖÐ£¬Ò³´íÎóÒÑ¾­·¢Éú£¬µÈ´ýÒ³×¼±¸ºÃ
         *    to wait till the page is ready
         * 3. Fault - page is not in cache, we have to generate a page fault
         Ò³²»ÔÚ»º´æÖÐ£¬ÐèÒª´¥·¢Ò³´íÎó

         Òª»ñÈ¡µÄÒ³Ãæ¿ÉÄÜ´æÔÚÈýÖÖ×´Ì¬£º
            1.ÒÑ¾­ÔÚ»º´æÖÐ£»
            2.ÒÑ¾­ÏÂ·¢»ñÈ¡£¬µ«ÊÇ»¹Ã»ÓÐ»ñÈ¡µ½£»
            3.Ã»ÓÐÔÚ»º´æÖÐ£¬ÐèÒª»ñÈ¡£»
         */
        ...
        while (trav_offset < rounded_end) {
                /*
                Õë¶ÔÔÚ»º´æÖÐµÄÒ³Ãæ£¬»¹ÒªÐ£Ñé»º´æÊÇ·ñÊ§Ð§£¬Ä¬ÈÏÓÐÐ§ÆÚÊÇ1ÃëÒÔÄÚ¡£
                Çé¿öÒ»£º
                Ò³ÃæÒÑ¾­ÔÚ»º´æÖÐ£¬ÇÒÓÐÐ§£»
                ÕâÀïÒ²·ÖÁ½ÖÖÇé¿ö£º
                Ò»ÖÖÊÇÒ³ÃæÓÐ»º´æÇÒÓÐÐ§£¬inodeÖÐÎÞwaitq£¨Ã»ÓÐÔÚ½øÐÐÐ£Ñé£©£¬Ö»ÐèÒªÌî³ä¼´¿É£»
                µÚ¶þÖÖÊÇÒ³ÃæÓÐ»º´æÇÒÓÐÐ§£¬ÇÒinodeÖÐÓÐwaitq£¨ÓÐÐ£Ñé£©;ÐèÒªwait on page£¬wait on inode;
                Çé¿ö¶þ£º
                Ò³ÃæÒÑ¾­ÔÚ»º´æÖÐ£¬¹ýÆÚÁË£»
                Ò²´æÔÚÁ½ÖÖÇé¿ö£º
                InodeÖÐÓÐwaitq£¨ÓÐÐ£Ñé£©ºÍÎÞwaitq£¨ÎÞÐ£Ñé£©µÄÇé¿ö£º
                ÓÐwaitq£¬Óë»º´æÓÐÐ§ÓÐwaitqÁ÷³ÌÒ»Ñù£»
                ÎÞwaitq£¬Á÷³ÌÈçÏÂ£
                Çé¿öÈý£º
                Ò³ÃæÒÑ¾­ÏÂ·¢£¬Î´·µ»Ø
                Çé¿öËÄ£ºÒ³Ãæ²»´æÔÚ£¬ÐèÒªÏÂ·¢£»
                ...
                */
                ...
                ioc_inode_lock (ioc_inode);
                {
                        ...
                        // µ±Ç°Òª»ñÈ¡Êý¾ÝµÄoffset£¬¸Õ¿ªÊ¼trav_offset¿ÉÄÜ»á±ÈoffsetÐ¡
                        //ÒòÎªtrav_offsetÓÐÏòÏÂÈ¡Õû
                        local_offset = max (trav_offset, offset);
                        // µ±Ç°Òª»ñÈ¡Êý¾ÝµÄ´óÐ¡£¬×î´óÒ»´ÎÒ»Ò³µÄÊý¾Ý£¬µÚÒ»´ÎºÍ×îºóÒ»´Î¿ÉÄÜÐ¡ÓÚÒ»Ò³
                        ...
                        // Ìí¼Óµ½pageµÄµÈ´ý¶ÓÁÐÖÐ,Ò³´íÎóµÄÒ²Òª
                        __ioc_wait_on_page (trav, frame, local_offset,
                                            trav_size);
                        //Êý¾Ý×¼±¸ºÃÁË
                        if (trav->ready) {
                                /* page found in cache */
                                // ²»ÐèÒª¸úÐÂºÍinodeÃ»ÓÐµÈ´ý¶ÓÁÐ
                                if (!might_need_validate && !ioc_inode->waitq) {
                                         ...
                                         * revalidate is
                                         * already on the way
                                         Èç¹ûioc_inode->waitq´æÔÚ£¬fstatµ÷ÓÃ»¹ÔÚ½øÐÐÖÐ
                                         */
                                        ...
                                        // Ìí¼Óµ½inodeµÈ´ýµÄµÈ´ý¶ÓÁÐÖÐ£¬Èç¹ûpageÌí¼Ó¹ýÁË£¬²»ÓÃÖØ¸´Ìí¼Ó
                                        // Ò»¸öpageÖ»ÐèÒ»´Î»½ÐÑ£¬»á±éÀúËùÓÐframe
                                        ...
                                        if (ret < 0) {
                                                ...
                                                local->op_errno = -ret;
                                                need_validate = 0;
                                                //»½ÐÑÒ³µÈ´ý£¬ÕâÀï×÷ÓÃÖ÷ÒªÊÇÇå¿Õwaitq
                                                waitq = __ioc_page_wakeup (trav,
                                                                           trav->op_errno);
                                                ioc_inode_unlock (ioc_inode);
                ...
                // ²»ÐèÒª¸üÐÂºÍinodeÃ»ÓÐµÈ´ý¶ÓÁÐ(¸ø1109ÐÐµÄwaitqÊ¹ÓÃµÄ),ÆäËûwaitqÎªNULL
                ioc_waitq_return (waitq);
                waitq = NULL;
...
out:
        // ÕæÕýµÄ unwind £¬ ioc_waitq_return ÖÐµ÷ÓÃÖ»ÊÇÊ¹local->wait_count¼õ1(ÕâÑùÀí½âÓÐ´íÎó£¬¼ûÏÂÃæ)
        // Õâ¸öº¯Êý¿ÉÄÜ»á±È ioc_waitq_returnÏÈµ÷ÓÃ£¬ÀýÈçÒ³´íÎóÊ±£¬ioc_waitq_returnÊÇÔÚ»Øµ÷º¯ÊýÖÐµ÷ÓÃµÄ£¬
        // ²»¹ýÄÇ¸öÏÈµ÷ÓÃÃ»ÓÐ¹ØÏµ£¬µ½ÕâÀïÊ±£¬ËùÓÐpageµÄµÈ´ý¶ÓÁÐ¶¼´¥·¢ÁË£¬×îºóÒ»¸ö»áµ÷ÓÃioc_frame_unwind
...
init (xlator_t *this)
{
        ...
        num_pages = (table->cache_size / table->page_size)
                + ((table->cache_size % table->page_size) //ÓÐÓàÊý¾Í¼Ó1
                   ? 1 : 0);
        //·ÖÅä»º´æµÄÄÚ´æ³Ø
...
struct volume_options options[] = {
        ...
        { .key  = {"cache-size"},
          ...
          .min  = 4 * GF_UNIT_MB,
          .max  = 32 * GF_UNIT_GB,
          .default_value = "32MB", // ÏµÍ³×î¶à¶à»º´æµÄ´óÐ¡
          .description = "Size of the read cache."
        },
        { .key  = {"min-file-size"},
          .type = GF_OPTION_TYPE_SIZET,
          .default_value = "0", //Ä¬ÈÏÎªÎÄ¼þ´óÐ¡×îÐ¡Öµ²»ÏÞÖÆ
          .description = "Minimum file size which would be cached by the "
          "io-cache translator."
        },
        { .key  = {"max-file-size"},
          .type = GF_OPTION_TYPE_SIZET,
          .default_value = "0",//Ä¬ÈÏÎªÎÄ¼þ´óÐ¡×î´óÖµ²»ÏÞÖÆ
